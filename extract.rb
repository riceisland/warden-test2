...
# CHANGES
# -----------
# 2010/12/19: BY RedCap (http://akr97.com/capsule/)
#  - Fixed regular expression and string for ruby1.9
...
module ExtractContent
  ...
  PUNCTUATIONS = Regexp.new(
                   %W[\343\200\201 \343\200\202 \357\274\201
                      \357\274\214 \357\274\216 \357\274\237].join('|') + 
                   '|\.[^A-Za-z0-9]|,[^0-9]|!|\?')


  RULED_LINES = Regexp.new(
                  '[' + 
                  %W[\342\224\200-\342\224\277 \342\225\200-\342\225\277].join + 
                  ']')

  USELESS_SYMBOLS = Regexp.new(
                      '[' +
                      %W[\342\200\230-\342\200\235 \342\206\220-\342\206\223 
                         \342\226\240-\342\226\275 \342\227\206-\342\227\257 
                         \342\230\205-\342\230\206].join +
                      ']')

  # Default option parameters.
  DEFAULT = {
		:threshold => 100,																				# threhold for score of the text
		:min_length => 25,																				# minimum length of evaluated blocks
		...
		:punctuations => PUNCTUATIONS,                            # punctuation characters
  ...
  class Extractor < Module
    ...
    def analyse(html, opt=nil)
      ...
      html.gsub!(/(<h\d\s*>\s*(.*?)\s*<\/h\d\s*>)/i) do |m|
        if $2.length >= 1 && title.include?($2) then "<div>#{$2}</div>" else $1 end
      end
    ...
    def eliminate_link(html)
      ...
      return "" if notlinked.length < 8 * count || islinklist(html)
      return notlinked
    end
    ...
    def strip_tags(html)
      ...
      st.gsub!(/[\357\274\201-\357\274\272]/){|c| (c.unpack('C*').last - 96).chr} # symbols, 0-9, A-Z
      st.gsub!(/[\357\275\201-\357\275\232]/){|c| (c.unpack('C*').last - 32).chr} # a-z
      st.gsub!(RULED_LINES, '') # keisen
      st.gsub!(/\343\200\200/, ' ')
