...
def rand_id_sample(app)
  ...
  case app
    ...
    when "flickr"
      ids = Flickr_photos.select(:id).filter(:user_id => current_user.id)
...
def one_data_create(app, id)
  ...
  case app
	...
	when "flickr"
	  ...
	  @flickr = Flickr.new(flickr_oauth.flickr_access_token, flickr_oauth.flickr_access_token_secret)
      #@user = @flickr.test_login
      
      data_hash = flickr_data_create(id)
...
def flickr_db_create(token, secret)
  ...
  list.each do |photo|
    ...
    past_photo = Flickr_photos.select(:id).filter(:user_id => current_user.id, :data_id => photo.id)
...
def reject(app)
  ...
  case app
    ...
    when "flickr"
	  Flickr_oauth.where(:uid => current_user.id).delete
	  Flickr_photos.where(:user_id => current_user.id).delete
	  Tags.where(:user_id => current_user.id, :app => "flickr").delete
...
get '/flickr_set' do
  ...
  if flickr_oauth
    ...
    flickr_db_create(session[:flickr_access_token],session[:flickr_access_token_secret])
